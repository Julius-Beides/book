...
crate with the semantic version specifier `0.6.1`. Cargo understands [Semantic
Versioning][semver]<!-- ignore --> (sometimes called *SemVer*), which is a
standard for writing version numbers. The number `0.6.1` is actually shorthand
for `^0.6.1`, which means “any version that has a public API compatible with
version 0.6.1.”
...
$ cargo build
  ...
  Downloaded semver-parser v0.7.0                
   ...
   Compiling guessing_game v0.1.0 (/projects/guessing_game)
    Finished dev [unoptimized + debuginfo] target(s) in 8.46s
```
...
happens if next week version 0.6.2 of the `rand` crate comes out and
...
remain at `0.6.1` until you explicitly upgrade, thanks to the *Cargo.lock*
file.
...
than `0.7.0`. If the `rand` crate has released two new versions, `0.6.2` and
...
that the version of the `rand` crate you are now using is `0.6.2`.
